...
export function LiveLog({ lines }: LiveLogProps) {
  return (
    <div className="glass rounded-2xl overflow-hidden mb-6">
      ...
      <div className="p-5 code-block max-h-48 overflow-y-auto space-y-1 font-mono text-sm" style={{ background: 'var(--code-bg)' }}>
        {lines.map((line) => (
          <div key={line}>{line}</div>
        ))}
        <div className="animate-pulse" style={{ color: 'var(--accent-blue)' }}>▊</div>
      </div>
    </div>
  );
}
