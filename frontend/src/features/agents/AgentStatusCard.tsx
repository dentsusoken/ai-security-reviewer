...
export function AgentStatusCard({
            ...
            <div>
              ...
              {chips.length ? (
                <div className="flex gap-2 mt-2 text-xs">
                  {chips.map((chip) => (
                    <span key={chip} className="px-2 py-0.5 rounded badge-medium">
                      {chip}
                    </span>
                  ))}
                </div>
  ...
  if (status === 'running') {
    return (
      <div
        className="rounded-2xl p-5 flex items-start gap-4 pulse-glow"
        style={{ background: 'rgba(79,139,255,0.08)', border: '1px solid rgba(79,139,255,0.4)' }}
      >
        <div
          className="w-11 h-11 rounded-xl flex items-center justify-center"
        ...
        <div className="flex-1">
          ...
          <ProgressBar
            ...
            gradient="linear-gradient(90deg, var(--accent-blue), var(--accent-cyan))"
          />
          <div className="text-xs mt-1 font-mono" style={{ color: 'var(--text-tertiary)' }}>
            {progress ?? 0}%
          </div>
  ...
  return (
    <div
      className="rounded-2xl p-5 flex items-start gap-4 opacity-50"
