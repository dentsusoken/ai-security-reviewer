    ...
    <div className="screen-content p-8 max-w-3xl">
      <div className="mb-6">
        <div className="flex items-center gap-2 text-xs mb-2" style={{ color: 'var(--text-tertiary)' }}>
          <PlusCircle className="w-3 h-3" /><span>NEW REVIEW</span>
        </div>
        <h1 className="text-3xl font-bold">新規セキュリティレビュー</h1>
        <p className="mt-2" style={{ color: 'var(--text-secondary)' }}>AIエージェントがコードを多角的に分析します</p>
      </div>
      ...
      {reviewType === 'url' ? (
        <UrlScanForm
          ...
          onChange={(field, value) => setUrlData((prev) => ({ ...prev, [field]: value as 'meta' | 'dns' | 'baseline' | 'full' }))}
        />
      ) : null}
      ...
      <div className="glass rounded-2xl p-6 mb-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="font-semibold">レビュー観点 <span className="text-xs font-normal" style={{ color: 'var(--text-tertiary)' }}>(複数選択可)</span></h2>
        </div>

        <div className="space-y-3">
          <label className="flex items-start gap-3 cursor-pointer p-3 rounded-xl border transition" style={{ borderColor: 'var(--border)' }}>
            <input type="checkbox" checked={aspects.asvs} onChange={(e) => setAspects((prev) => ({ ...prev, asvs: e.target.checked }))} className="mt-1" style={{ accentColor: 'var(--accent-blue)' }} />
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <span className="font-semibold text-sm">OWASP ASVS 網羅性チェック</span>
                <span className="text-xs px-2 py-0.5 rounded" style={{ background: 'rgba(79,139,255,0.2)', color: 'var(--accent-blue)' }}>推奨</span>
              </div>
              <div className="text-xs mt-1" style={{ color: 'var(--text-tertiary)' }}>AIエージェントが仕様観点でレビュー</div>
            </div>
          </label>

          <label className="flex items-start gap-3 cursor-pointer p-3 rounded-xl border transition" style={{ borderColor: 'var(--border)' }}>
            <input type="checkbox" checked={aspects.sast} onChange={(e) => setAspects((prev) => ({ ...prev, sast: e.target.checked }))} className="mt-1" style={{ accentColor: 'var(--accent-blue)' }} />
            <div className="flex-1">
              <div className="font-semibold text-sm">静的解析 (Semgrep)</div>
              <div className="text-xs mt-1" style={{ color: 'var(--text-tertiary)' }}>既知の脆弱性パターンをコード検索</div>
            </div>
          </label>

          <label className="flex items-start gap-3 cursor-pointer p-3 rounded-xl border transition" style={{ borderColor: 'var(--border)' }}>
            <input type="checkbox" checked={aspects.dast} onChange={(e) => setAspects((prev) => ({ ...prev, dast: e.target.checked }))} className="mt-1" style={{ accentColor: 'var(--accent-blue)' }} />
            <div className="flex-1">
              ...
              <div className="text-xs mt-1 flex items-center gap-1" style={{ color: '#EA580C' }}>
                <AlertTriangle className="w-3 h-3" />URL所有確認が必要
              </div>
            </div>
          </label>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {[
            ...
            return (
              ...
              >
                {item.recommended ? (
                  <span className="absolute -top-2 -right-2 text-xs px-2 py-0.5 rounded-full text-white" style={{ background: 'linear-gradient(135deg, var(--accent-blue), var(--accent-purple))' }}>
                    推奨
                  </span>
                ) : null}
                <div className="font-semibold text-sm">{item.title}</div>
                <div className="text-xs mt-1" style={{ color: 'var(--text-tertiary)' }}>{item.eta}</div>
              </button>
            );
          })}
        </div>
      ...
      <div className="flex justify-end gap-3">
        ...
        <button onClick={() => navigate('/dashboard')} className="px-6 py-3 rounded-xl border transition hover:opacity-80" style={{ borderColor: 'var(--border)' }}>
          キャンセル
        </button>
        <button
        ...
        >
          {isSubmitting ? (
            ...
            <>
              <Play className="w-4 h-4" />レビューを開始
            </>
          )}
        </button>
      </div>
