...
export function ResultPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const transformedFindings: FindingItem[] = findingsResponse.findings.map(
          (f: FindingSummary) => ({
            id: f.id,
  ...
  const dimensions =
    review.perspectiveScores?.slice(0, 5).map((d) => ({
      name: d.category,
      score: d.percentage,
    })) || [];
    ...
    <div className="screen-content p-8 max-w-5xl">
      <div className="flex justify-between items-start mb-8 gap-4 flex-wrap">
        <div>
          ...
          >
            ...
            <span className="flex items-center gap-1.5">
              ...
              {formatDuration(review.durationMs || 0)}
            </span>
            <span
              className="flex items-center gap-1.5 text-xs px-2 py-0.5 rounded-full"
              style={{ background: 'rgba(79,139,255,0.15)', color: 'var(--accent-blue)' }}
            >
              <FolderGit2 className="w-3 h-3" />
              {review.inputType === 'github'
                ? 'GitHub'
                : review.inputType === 'code'
                  ? 'Code'
                  : 'URL'}
            </span>
            {review.repoUrl && (
        ...
        <button
          ...
          className="btn-gradient px-4 py-2.5 rounded-xl font-semibold text-sm inline-flex items-center gap-2"
        >
      ...
      <div className="glass rounded-2xl p-8 mb-8 relative overflow-hidden">
        <div
          ...
          style={{ background: `${getScoreColor(score)}20` }}
        />
        <div className="relative grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <div className="flex items-baseline gap-3 mb-2">
              <span className="text-7xl font-extrabold" style={{ color: getScoreColor(score) }}>
                {score}
              </span>
            ...
            <div
              ...
              style={{ background: `${getScoreColor(score)}33`, color: getScoreColor(score) }}
            >
              {getScoreLabel(score)}
            </div>
          </div>
          ...
          <div className="space-y-2">
            {[
              ...
              <div
                key={item.label}
                className={`flex items-center justify-between p-3 rounded-xl ${item.cls}`}
              >
        ...
        <>
          ...
          <div className="glass rounded-2xl p-6 mb-8 space-y-4">
            {dimensions.map((item) => {
              ...
              const gradient =
                item.score >= 80
                  ...
                  : item.score >= 60
                    ? 'linear-gradient(90deg, #EA580C, #FACC15)'
            ...
            {review.perspectiveScores && review.perspectiveScores.length > 5 && (
              <div className="text-xs pt-1 text-center" style={{ color: 'var(--text-tertiary)' }}>
                ▼ 残り{review.perspectiveScores.length - 5}項目を展開
              </div>
            )}
          </div>
        </>
      ...
      <FindingsList
        findings={findings}
        onSelect={(findingId) => navigate(`/reviews/${reviewId}/findings/${findingId}`)}
      />
      ...
      >
        <button
          onClick={() => navigate('/history')}
          className="px-5 py-2.5 rounded-xl border transition hover:opacity-80 text-sm inline-flex items-center gap-2"
          style={{ borderColor: 'var(--border)' }}
        >
        ...
        <button
          ...
          className="btn-gradient px-5 py-2.5 rounded-xl font-semibold text-sm inline-flex items-center gap-2"
        >
